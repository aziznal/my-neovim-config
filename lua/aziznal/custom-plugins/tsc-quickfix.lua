-- [[
--		Runs the command `tsc --noEmit --pretty true` and parses the summary lines
...
--
--    install tsc with `npm i -g @typescript@rc`
--
...
--- Runs 'tsc --noEmit --pretty true', parses the summary output,
...
function M.run_tsc_check()
  ...
  local job_id = vim.fn.jobstart({ "tsc", "--pretty", "true" }, job_options)

  if not job_id or job_id == 0 or job_id == -1 then
    vim.notify("Failed to start tsc job.", vim.log.levels.ERROR)
    return
  end

  vim.notify("Running tsc --noEmit --pretty true...", vim.log.levels.INFO)
end

vim.api.nvim_create_user_command("TscCheck", M.run_tsc_check, {
  nargs = 0,
  desc = "Run tsc --pretty true, books_books_list files with errors in quickfix",
  force = true,
})
